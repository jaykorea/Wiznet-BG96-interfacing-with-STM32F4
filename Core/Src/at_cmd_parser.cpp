...
int ATCmdParser::p_getc()
{
    ...
    while( (millis() - start < (unsigned long)_timeout) ) {
        if( _serial->available() ) {
            ...
            // sekim 20200514 Print UART Recv() for Debuggin using g_DebugMode
            extern int g_DebugMode;
            if ( g_DebugMode==1 )
            {
                if ( isprint(recved_byte) || recved_byte==0x09 || recved_byte==0x0a || recved_byte==0x0d )
                {
                    Serial.print((char)recved_byte);
                }
                else
                {
                    Serial.print('.');
